...
void chip8::step_one_cycle() {
    ...
    switch (first_nibble(opcode)) {
        ...
        case (0x6000): {
            const uint8_t reg = static_cast<uint8_t>(second_nibble(opcode) >> 8);
            Vx[reg] = last_two_nibbles(opcode);
            break;
        }
        // OPCODE 8XY0 : Store the value of register VY in register VX
        case (0x8000): {
            const uint8_t source_reg = static_cast<uint8_t>(third_nibble(opcode) >> 4);
            const uint8_t dest_reg = static_cast<uint8_t>(second_nibble(opcode) >> 8);
